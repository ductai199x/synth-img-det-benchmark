...
def to_sym(x, x_min, x_max, L):
    """
    Converts a tensor `x` to a symmetric range based on the given minimum and maximum values.

    Args:
        x (torch.Tensor): The input tensor to be converted.
        x_min (float): The minimum value of the symmetric range.
        x_max (float): The maximum value of the symmetric range.
        L (int): The number of levels in the symmetric range.

    Returns:
        torch.Tensor: The tensor `x` converted to the symmetric range.
    """
...
def to_bn(S, x_min, x_max, L):
    """
    Converts a tensor of symbols to a tensor of bin values.

    Args:
        S (torch.Tensor): A tensor of symbols.
        x_min (float): The minimum value of the range.
        x_max (float): The maximum value of the range.
        L (int): The number of levels or bins.

    Returns:
        torch.Tensor: A tensor with the same shape as S, where each symbol is converted to a bin value.
    """
    sym_range = x_max - x_min
    bin_size = sym_range / (L - 1)
...
class DiscretizedMixLogisticLoss(nn.Module):
    ...
    def to_sym(self, x):
        return to_sym(x, self.x_min, self.x_max, self.L)

    def to_bn(self, S):
        return to_bn(S, self.x_min, self.x_max, self.L)
    ...
    def log_cdf(self, lo, hi, means, log_scales):
        ...
        assert lo.min() >= self.x_min and hi.max() <= self.x_max, "{},{} not in {},{}".format(
        ...
        normalized_lo = inv_stdv * (centered_lo - self.bin_width / 2)  # sigma' * (x - mu - 1/255)
        ...
        assert not torch.any(log_cdf_delta > 1e-6), f"{log_cdf_delta[log_cdf_delta > 1e-6]}"
    ...
    def _extract_non_shared_c(
        self, c: int, C: int, l: torch.Tensor, x: Optional[torch.Tensor] = None
        ...
        if self.use_coeffs and c != 0:
            ...
            if c == 1:
                ...
                coeffs_g_r = self._nonshared_coeffs_act(unscaled_coeffs[:, 0, ...])  # NKHW
                means_c += coeffs_g_r * x[:, 0, ...]
            elif c == 2:
                assert x is not None
                coeffs_b_r = self._nonshared_coeffs_act(unscaled_coeffs[:, 1, ...])  # NKHW
                coeffs_b_g = self._nonshared_coeffs_act(unscaled_coeffs[:, 2, ...])  # NKHW
    ...
    def _non_shared_sample(self, l, C):
        ...
        u = torch.zeros_like(logit_probs).uniform_(1e-5, 1.0 - 1e-5)  # NCKHW
        # argmax over K, results in NCHW,
        # specifies for each c: which of the K mixtures to take
        sel = torch.argmax(logit_probs - torch.log(-torch.log(u)), dim=2)  # gumbel sampling
        ...
        u = torch.zeros_like(means).uniform_(1e-5, 1.0 - 1e-5)  # NCHW
        x = means + torch.exp(log_scales) * (torch.log(u) - torch.log(1.0 - u))  # NCHW
