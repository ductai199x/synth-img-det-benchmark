...
from ..common_transforms_utils import Compose, Normalize, center_crop_no_pad, partial
...
class CnnDetEvalWrapper(BaseEvalWrapper):
    def metrics(self):
    ...
    def transforms_fn(self, x):
        if self.transforms is None:
            self.transforms = Compose(
                [
                    partial(center_crop_no_pad, size=224),
