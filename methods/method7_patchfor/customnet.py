...
from timm.models.xception import SeparableConv2d, Block as XceptionBlock
...
def make_xceptionnet_long():
    ...
    def change_out(layers):
        ...
        new_layers = [
            ...
            (
                "pblock3",
                PixelBlock(channels[4], channels[5], 2, 1, start_with_relu=True, grow_first=True),
            ),
            (
                "pblock4",
                PixelBlock(channels[5], channels[6], 3, 1, start_with_relu=True, grow_first=True),
            ),
        ]
        num_ch = channels[9]
...
class CustomResNet(nn.Module):
    ...
    def __init__(
        ...
        sequence = modify_sequence(
            [
                ...
                ("maxpool", nn.MaxPool2d(3, stride=1 if halfsize else 2, padding=1)),
    ...
    def _make_layer(self, block, channels, depth, stride=1):
        return ResNet._make_layer(self, block, channels, depth, stride)
...
class CustomXceptionNet(nn.Module):
    """
    Customizable Xceptionnet, compatible with https://github.com/Cadene/pretrained-models.pytorch/blob/master/pretrainedmodels/models/py
    but:
     * The top-level sequence of modules can be modified to add
       or remove or alter layers.
     * Extra outputs can be produced, to allow backprop and access
    ...
    def __init__(
        ...
        sequence = modify_sequence(
            [
                ...
                (
                    "block1",
                    XceptionBlock(channels[2], channels[3], 2, 2, start_with_relu=False, grow_first=True),
                ),
                (
                    "block2",
                    XceptionBlock(channels[3], channels[4], 2, 2, start_with_relu=True, grow_first=True),
                ),
                (
                    "block3",
                    XceptionBlock(channels[4], channels[5], 2, 2, start_with_relu=True, grow_first=True),
                ),
                (
                    "block4",
                    XceptionBlock(channels[5], channels[6], 3, 1, start_with_relu=True, grow_first=True),
                ),
                (
                    "block5",
                    XceptionBlock(channels[6], channels[7], 3, 1, start_with_relu=True, grow_first=True),
                ),
                (
                    "block6",
                    XceptionBlock(channels[7], channels[8], 3, 1, start_with_relu=True, grow_first=True),
                ),
                (
                    "block7",
                    XceptionBlock(channels[8], channels[9], 3, 1, start_with_relu=True, grow_first=True),
                ),
                (
                    "block8",
                    XceptionBlock(channels[9], channels[10], 3, 1, start_with_relu=True, grow_first=True),
                ),
                (
                    "block9",
                    XceptionBlock(channels[10], channels[11], 3, 1, start_with_relu=True, grow_first=True),
                ),
                (
                    "block10",
                    XceptionBlock(channels[11], channels[12], 3, 1, start_with_relu=True, grow_first=True),
                ),
                (
                    "block11",
                    XceptionBlock(channels[12], channels[13], 3, 1, start_with_relu=True, grow_first=True),
                ),
                (
                    "block12",
                    XceptionBlock(channels[13], channels[14], 2, 2, start_with_relu=True, grow_first=False),
                ),
                ("conv3", SeparableConv2d(channels[14], channels[15], 3, 1, 1)),
                ("bn3", nn.BatchNorm2d(channels[15])),
                ("relu3", nn.ReLU(inplace=True)),
                ("conv4", SeparableConv2d(channels[15], channels[16], 3, 1, 1)),
