...
from .lpips_nets import vgg16, alexnet, squeezenet
from .ops import normalize_tensor, tensor2im, tensor2tensorlab, tensor2np, l2, dssim
...
class LPIPS(nn.Module):
            ...
            if pretrained:
                if model_path is None:
                    ...
                    model_path = os.path.abspath(
                        os.path.join(
                            inspect.getfile(self.__init__), "..", "weights/v%s/%s.pth" % (version, net)
                        )
                    )
    ...
    def forward(self, in0, in1, retPerLayer=False, normalize=False):
        ...
        if self.lpips:
            ...
            if self.spatial:
                res = [
                    upsample(diffs[kk].sum(dim=1, keepdim=True), out_HW=in0.shape[2:]) for kk in range(self.L)
                ]
            else:
                res = [
