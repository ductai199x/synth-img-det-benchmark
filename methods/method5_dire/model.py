...
from ..common_transforms_utils import Compose, CenterCrop, Normalize, Resize
...
class DireModel(nn.Module):
    def __init__(self, unet: UNetModel, diffusion: SpacedDiffusion, classifier: nn.Module):
        super().__init__()
        self.unet = unet
    ...
    def get_dire(self, x: torch.Tensor, recons: torch.Tensor):
        dire = (x - recons).abs()
        dire = dire.mul(127.5).clamp(0, 255).to(torch.uint8).float().div(255)
        return dire

    def forward(self, x: torch.Tensor):
        x = x.mul(255).div(127.5).sub(1)
