...
from ..common_transforms_utils import Compose, Normalize, CenterCrop, interpolate, center_crop_no_pad, partial
...
class PatchForEvalWrapper(BaseEvalWrapper):
    def metrics(self):
    ...
    def transforms_fn(self, x):
        if self.transforms is None:
            self.transforms = Compose(
                [
                    partial(center_crop_no_pad, size=512),
                    partial(interpolate, mode="bilinear", scale_factor=2.0),
                    partial(center_crop_no_pad, size=512),
