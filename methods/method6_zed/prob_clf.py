...
class StackedAtrousConvs(nn.Module):
    def __init__(
        self, atrous_rates_str: Union[str, int], Cin: int, Cout: int, bias: bool = True, kernel_size: int = 3
    ) -> None:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # type: ignore
        x = torch.cat([atrous(x) for atrous in self.atrous], dim=1)  # type: ignore
        x = self.lin(x)
        return x
